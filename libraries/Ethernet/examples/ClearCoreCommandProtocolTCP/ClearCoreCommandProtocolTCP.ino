...
// MAC address of the ClearCore. Arduino ethernet functions require 
// something to be provided here, but it isn't actually used
byte mac[] = {0x24, 0x15, 0x10, 0xB0, 0x05, 0x8b};

// The IP address of the server to which to connect (not ClearCore's IP address)
...
//give the user time to open the USB port to view the IP address printout
  ...
  if (usingDhcp) {
    ...
    //Ethernet.begin(mac, ip, dns, gatewayAddr, subnetAddr);
  ...
  if (!client.connect(serverIp, PORT_NUM)) {
    ...
    client.println("Setup successful");
    client.println("Send 'h' to receive a list of valid commands"); 
  }
	//==================================================================
	//========================/Ethernet TCP Setup========================
	//==================================================================
...
//initialize the connectionDelay timer
uint32_t startTime = millis();
...
void loop() {
    ...
    //  from another source than the default TCP input, ensure that the command is stored 
    ...
    if (i==0){
      ...
      // buffer overflow (there is no space left in the buffer, but there are still characters to read from the input stream)
      ...
      // read and discard any other characters in the input stream
      while(client.peek()!=-1){
        client.read();
			  Delay_ms(1);
      }
    ...
    if (inputValid){
      ...
      switch(input[0]){
        ...
        case 'q':
          ...
          switch(input[2]){
            ...
            case 'P':
              ...
              if (verboseFeedback){
                client.print("Motor ");
                client.print(motorNum_in);
                client.print(" is in position (steps) ");
              }
              client.println(queriedValue);
              break;
            ...
            case 'V':
              ...
              if (verboseFeedback){
                client.print("Motor ");
                client.print(motorNum_in);
                client.print(" is at velocity (steps/s) ");
              }
              client.println(queriedValue);
              break;
            ...
            case 'S':
              ...
              if (verboseFeedback){
                ...
                client.println(statusReg.reg, 2);  // prints the status register in binary
                if (statusReg.bit.AlertsPresent){
                  client.println(alertReg.reg, 2); // prints the alert register in binary
                }
              }
              break;
        ...
        case 'i':
          ...
          if(verboseFeedback){
            client.print("Connector ");
            client.print(connectorNum_in);
            client.print(" value: ");
          }
          client.println(inputConnectorValue);
          break;
...
*/
 void SendFeedback(int32_t messageNumber){
  ...
  if (verboseFeedback){
    client.println(FeedbackMessages[messageNumber].message);
  } else {
    client.println(FeedbackMessages[messageNumber].number);
  }
 } // SendFeedback()
//------------------------------------------------------------------------------
...
void SendVerboseStatus(int32_t motorNumber) {
  ...
  client.print("Motor status register for motor M");
  client.print(motorNumber);
  client.print(": ");
  client.println(statusReg.reg, 2); // prints the status register in binary 
  
  client.print("AtTargetPosition:  ");
  client.println(statusReg.bit.AtTargetPosition);

  client.print("StepsActive:       ");
  client.println(statusReg.bit.StepsActive);

  client.print("AtTargetVelocity:  ");
  client.println(statusReg.bit.AtTargetVelocity);

  client.print("MoveDirection:     ");
  client.println(statusReg.bit.MoveDirection);

  client.print("MotorInFault:      ");
  client.println(statusReg.bit.MotorInFault);

  client.print("Enabled:           ");
  client.println(statusReg.bit.Enabled);

  client.print("PositionalMove:    ");
  client.println(statusReg.bit.PositionalMove);

  client.print("HLFB State:    ");
  switch (statusReg.bit.HlfbState) {
    case 0:
      client.println("HLFB_DEASSERTED");
      break;
    case 1:
      client.println("HLFB_ASSERTED");
      break;
    case 2:
      client.println("HLFB_HAS_MEASUREMENT");
      break;
    case 3:
      client.println("HLFB_UNKNOWN");
      break;
    default:
  ...
  client.print("Triggering:        ");
  client.println(statusReg.bit.Triggering);

  client.print("InPositiveLimit:   ");
  client.println(statusReg.bit.InPositiveLimit);

  client.print("InNegativeLimit:   ");
  client.println(statusReg.bit.InNegativeLimit);

  client.print("InEStopSensor:     ");
  client.println(statusReg.bit.InEStopSensor); 

  client.println("--------------------------------");
  
    
  if (statusReg.bit.AlertsPresent){
    client.print("Alert register:  ");
    client.println(alertReg.reg, 2); // prints the alert register in binary

    client.print("MotionCanceledInAlert:         ");
    client.println(alertReg.bit.MotionCanceledInAlert);

    client.print("MotionCanceledPositiveLimit:   ");
    client.println(alertReg.bit.MotionCanceledPositiveLimit);

    client.print("MotionCanceledNegativeLimit:   ");
    client.println(alertReg.bit.MotionCanceledNegativeLimit);

    client.print("MotionCanceledSensorEStop:     ");
    client.println(alertReg.bit.MotionCanceledSensorEStop);

    client.print("MotionCanceledMotorDisabled:   ");
    client.println(alertReg.bit.MotionCanceledMotorDisabled);

    client.print("MotorFaulted:                  ");
    client.println(alertReg.bit.MotorFaulted);
